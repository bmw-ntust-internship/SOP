...
| `daily-log.md` | Daily tracking format + Auto Daily-Log workflow (Prompt A: MySQL; Prompt B: 4-file) |
...
| `lab-automation/llm-memory.md` | MySQL long-term memory setup for LLM sessions |
...
| BMW Lab VM | `140.118.122.119:3306` | MySQL `llm_memory` DB for long-term LLM memory |
...
Defined in `daily-log.md §Auto Daily-log`. Two modes:

| Mode | When | What it does |
| --- | --- | --- |
| **Prompt A** (MySQL) | BMW Lab VM reachable via SSH tunnel | Inserts session/prompt records into `llm_memory` DB; derives commit summary from DB |
| **Prompt B** (4-file) | Offline or tunnel unavailable | Reconciles CLAUDE.md, CONTEXT.md, MEMORY.md, TODO.md; commits all 4 |
...
- Commit format: Title / Work Start (`yyyy/mm/dd: hh.mm`) / Summary / Details / Co-authored-by
